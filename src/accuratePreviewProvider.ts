...
export class AccurateQuarkdownPreviewProvider implements vscode.WebviewPanelSerializer {
    ...
    private preprocessQuarkdownSyntax(content: string): string {
        ...
        processed = processed.replace(
            /^\.var\s*\{([^}]+)\}\s*\{([^}]*)\}\s*$/gm,
            '<div class="qmd-variable-def"><span class="qmd-keyword">var</span> <span class="qmd-name">$1</span> = <span class="qmd-value">$2</span></div>'
        );

        // Process function calls with named parameters
        processed = processed.replace(
            /\.(\w+)\s*\{([^}]*)\}\s*([^{}\n]*?):/g,
            (match, funcName, arg, namedParams) => {
        ...
        processed = processed.replace(
            /^\.include\s*\{([^}]+)\}\s*$/gm,
            '<div class="qmd-include"><span class="qmd-keyword">include</span> <span class="qmd-filename">$1</span></div>'
        );
        ...
        processed = processed.replace(
            /^\.doctype\s*\{([^}]+)\}\s*$/gm,
            '<div class="qmd-doctype"><span class="qmd-keyword">doctype</span> <span class="qmd-doctype-name">$1</span></div>'
        );

        processed = processed.replace(
            /^\.pageformat\s*\{([^}]+)\}(?:\s*orientation:\{([^}]+)\})?\s*$/gm,
    ...
    private processControlStructures(content: string): string {
        let processed = content;

        // Process if statements
        processed = processed.replace(
            /^\.if\s*\{([^}]+)\}\s*$/gm,
            '<div class="qmd-if"><span class="qmd-keyword">if</span> <span class="qmd-condition">$1</span></div>'
        );

        // Process ifnot statements
        processed = processed.replace(
            /^\.ifnot\s*\{([^}]+)\}\s*$/gm,
            '<div class="qmd-ifnot"><span class="qmd-keyword">ifnot</span> <span class="qmd-condition">$1</span></div>'
        );

        // Process foreach loops
        processed = processed.replace(
            /^\.foreach\s*\{([^}]+)\}\s*(\w+):\s*$/gm,
            '<div class="qmd-foreach"><span class="qmd-keyword">foreach</span> <span class="qmd-range">$1</span> <span class="qmd-variable">$2</span>:</div>'
        );

        // Process repeat loops
        processed = processed.replace(
            /^\.repeat\s*\{([^}]+)\}\s*(\w+):\s*$/gm,
            '<div class="qmd-repeat"><span class="qmd-keyword">repeat</span> <span class="qmd-count">$1</span> <span class="qmd-variable">$2</span>:</div>'
        );

        return processed;
    }

    private processLayoutFunctions(content: string): string {
        let processed = content;
        ...
        processed = processed.replace(
            /^\.center(?:\s*:)?\s*$/gm,
            '<div class="qmd-layout qmd-center"><span class="qmd-keyword">center</span>:</div>'
        );
    ...
    private renderMarkdownContent(content: string): string {
        ...
        if (this._currentSettings.enableMath) {
            processed = processed.replace(
                /\$\$([\s\S]*?)\$\$/g,
                '<div class="qmd-math-block">$$1$$</div>'
            );
            
            processed = processed.replace(
                /\$([^\$\n]+)\$/g,
                '<span class="qmd-math-inline">$$1$</span>'
            );
        }
    ...
    private enhanceRenderedHtml(html: string): string {
