...
export class EnhancedQuarkdownPreviewProvider implements vscode.WebviewPanelSerializer {
    private _panels = new Map<string, vscode.WebviewPanel>();
    private _watchers = new Map<string, fs.FSWatcher>();
    private _currentSettings: PreviewSettings = {
        theme: 'darko',
        layout: 'minimal',
        enableMath: true,
        enableWatch: true
    ...
    private getFallbackHtml(content: string): string {
        ...
        processedContent = processedContent
            .replace(/\.(sum|multiply|divide|pow|sin|cos|tan|truncate|round|iseven|isgreater|pi)\s*([^\n]*)/g, 
                '<span class="qmd-math-function">🔢 $1($2)</span>');
