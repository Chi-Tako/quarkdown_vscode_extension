...
export async function createProject(template: ProjectTemplate, targetPath: string): Promise<void> {
...
export async function showProjectCreationDialog(): Promise<void> {
    // Show template selection
    const templateItems = PROJECT_TEMPLATES.map(template => ({
        label: template.name,
    ...
    try {
        ...
        const openProject = await vscode.window.showInformationMessage(
            `Quarkdown project "${projectName}" created successfully!`,
            'Open Project', 'Open Main File'
        );
...
export function getProjectCommands(): vscode.Disposable[] {
    return [
...
Conclusion for ${chapterName}...`;
            ...
            const openChapter = await vscode.window.showInformationMessage(
...
# Add your custom functions here...`;
            ...
            const openLib = await vscode.window.showInformationMessage(
                `Library file "${libraryName}" created successfully!`,
                'Open Library'
            );
